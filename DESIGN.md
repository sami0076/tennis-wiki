...
- **Simulator:** the two player pickers and the surface and length controls above the chain; the draw panel below carries its own title and a `DrawPicker` on the same line, so which draw is being replayed is a control rather than a caption. The picker sits above every state of that panel, the declined ones included: being told this draw cannot be replayed is exactly when somebody wants a different one.
