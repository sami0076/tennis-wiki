...
- **Simulator:** the two player pickers and the surface and length controls above the chain; the draw panel below carries its own title and a `DrawPicker` on the same line, so which draw is being replayed is a control rather than a caption. It is a plain labelled dropdown, not a pill in a tray -- the tray controls on this page are toggles and this one opens a list. The picker sits above every state of that panel, the declined ones included: being told this draw cannot be replayed is exactly when somebody wants a different one. It always says why it is empty when it is, because a disabled select with no explanation reads as a broken one. The page opens on the top of that list rather than a hardcoded draw.
- **A rating line is cut where the player went unrated**, rather than joined across the hole. The ratings table holds a snapshot per week played and nothing for the weeks not played, so a segment drawn across an absence asserts a climb or slide that never happened. The cut is declared in the caption and in the chart's description; a week left alone between two absences is drawn as a dot, since a line of one point draws nothing. What counts as an absence is six months, raised to four times the line's own usual spacing where that is longer -- a series sampled once a year is annual, not absent, and a fixed rule shattered it into dots.
- **Both axes on a chart are numbered.** The ruling sits on round ratings inside the range rather than on fifths of it, because 2600 and 2500 are figures a reader already holds and 2547 is arithmetic. The time axis names whole years over a long window and month starts over a season, carrying the year on the first tick and wherever it turns. Tick labels are HTML over the plot, never text inside it: the SVG is stretched to the column with `preserveAspectRatio="none"`, which would stretch any glyph drawn in it.
- Everything is one column below 880px, and a table wider than a phone scrolls inside its card.
- **Every `<option>` and `<optgroup>` carries its own background and colour**, set once in `base.css` rather than per select. The open list of a `<select>` is painted by the platform and takes its colours from the select, so a select styled to look like part of a card -- no border, no background -- hands the popup a transparent background that the platform resolves to its own white canvas. In dark mode the option text is near-white, so the list read white on white. Setting the pair on the options leaves each select free to look like whatever its surroundings need while the list it opens stays readable in both themes.
...
- **Charts:** lines draw in behind a clip that widens from the left, never a stroke dash. A dash is measured along the stroke while `pathLength` normalises along the path, and under a plot stretched to the column width the two disagree: the reveal painted the two ends before the middle, which on a rating chart is indistinguishable from a hole in the data and was reported as one. A clip is geometry, so it reveals in order, and with no animation running the rectangle is simply full size -- an animation must never be able to hide data at rest. Area washes fade in after them; peak and end markers pop. Once drawn they are read rather than admired: pointing at an `AreaChart`, or focusing it and pressing an arrow, puts a crosshair on the nearest week and writes its date and rating over the line. Nothing about the readout animates -- a tooltip that faded would lag the pointer it is answering.
- **Radar:** two players over shared percentile axes (ADR-0015). A is a solid violet outline and B a dashed magenta one, each over a 10% wash; the dashed ring is the tour median. A figure a player lacks leaves a dotted chord, never a vertex at the centre. The shapes pop from the centre on arrival; pointing at an axis, or pressing an arrow, reads both players below the plot.
