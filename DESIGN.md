...
The home page's first viewport and the site's one piece of ambient motion. Left, the eight leaders' form lines over 24 months on the ruling, the top three in the ink ramp and the field in pencil-mid at 0.6 opacity; a dotted lead (`1 3`) carries a line that stopped early to the edge. A 40px gutter holds an L-shaped ruled step from each line's end to its row. Right, the seed rows: `[seed] Name (CTY) ····· Elo delta`, 40px tall on pencil rules, a dotted leader between name and figure, the list opened and closed by ink rules. Hover on a row lights its line to ink and dims the others to pencil-light and their rows to 0.55 opacity; the row itself goes highlight. On a phone the two stack, the steps are not drawn, and the top three lines carry `[1]` `[2]` `[3]` at their ends instead.
